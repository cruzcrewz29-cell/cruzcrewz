<script lang="ts">
	...
	async function toggleFeatured(id: string, currentValue: boolean) {
		await supabase.from('testimonials').update({ featured: !currentValue }).eq('id', id);
		await loadTestimonials();
	}
</script>
...
<div class="space-y-6">
	...
	{#if testimonials.length === 0}
		<div class="rounded-xl border-2 border-dashed border-gray-300 bg-white p-12 text-center">
			...
			<button
				onclick={openAddModal}
				class="mt-6 inline-flex items-center gap-2 rounded-lg bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800"
			>
				<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
					<path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
				</svg>
					...
					<div class="flex items-start justify-between">
						...
						<div class="ml-4 flex flex-col gap-2">
							...
							>
								<svg
									class="h-4 w-4"
									fill={testimonial.featured ? 'currentColor' : 'none'}
									viewBox="0 0 20 20"
									stroke="currentColor"
									stroke-width="1.5"
								>
			...
			<div class="relative w-full max-w-2xl rounded-xl bg-white shadow-xl">
				...
				<form onsubmit={handleSubmit} class="p-6">
					<div class="space-y-4">
						<div class="grid gap-4 sm:grid-cols-2">
							<div>
								...
								<input
									...
									required
									class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-gray-900 focus:ring-gray-900"
						...
						<div>
							<label for="testimonial-text" class="block text-sm font-medium text-gray-700"
								>Testimonial</label
							>
							<textarea
								id="testimonial-text"
