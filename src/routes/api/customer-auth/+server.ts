// src/routes/api/customer-auth/+server.ts
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const emailBody = {
      ...
      subject: 'Your Cruz Crewz sign-in link',
      content: [
        {
          type: 'text/html',
          value: `
...
<body style="margin:0;padding:0;background:#f9fafb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <div style="max-width:520px;margin:40px auto;padding:0 16px;">
    <div style="background:#fff;border-radius:16px;border:1px solid #e5e7eb;overflow:hidden;">
      ...
      <div style="padding:32px;">
        ...
        <p style="margin:0 0 24px;font-size:14px;color:#6b7280;line-height:1.6;">
          Click the button below to sign in to your Cruz Crewz account. This link expires in 24 hours.
        </p>
    ...
    if (!sgRes.ok) {
      console.error('[customer-auth] SendGrid error:', await sgRes.text());
    }

    return json({ success: true });
  } catch (err) {
    console.error('[customer-auth]', err);
    return json({ error: 'Failed to send sign-in link' }, { status: 500 });
  }
};